...
def test_surface_transformation_is_unimodular(simple_cubic, miller):
    ...
    matrix = sl.surface_transformation(simple_cubic.lattice, miller)
    assert abs(round(float(np.linalg.det(matrix)))) == 1


def test_surface_transformation_rejects_zero_indices(simple_cubic):
    with pytest.raises(ValueError):
        sl.surface_transformation(simple_cubic.lattice, (0, 0, 0))
...
def test_slab_c_axis_follows_the_surface_normal(simple_cubic, miller):
    slab = sl.build_slab(simple_cubic, miller, layers=2, vacuum=10.0)
...
def test_slab_surface_area(simple_cubic, miller, area):
    slab = sl.build_slab(simple_cubic, miller, layers=1, vacuum=10.0)
    assert sl.surface_area(slab) == pytest.approx(area, rel=1e-9)
...
def test_slab_atom_count_scales_with_layers(simple_cubic):
    for layers in (1, 3, 5):
        slab = sl.build_slab(simple_cubic, (0, 0, 1), layers=layers, vacuum=10.0)
        assert len(slab.atoms) == layers


def test_slab_c_length_is_thickness_plus_vacuum(simple_cubic):
    slab = sl.build_slab(simple_cubic, (0, 0, 1), layers=3, vacuum=10.0)
...
def test_slab_vacuum_is_split_across_both_faces(simple_cubic):
    slab = sl.build_slab(simple_cubic, (0, 0, 1), layers=2, vacuum=12.0)
    normal = surface_normal(slab)
...
def test_slab_termination_shift_changes_the_exposed_plane():
    ...
    plain = sl.build_slab(cell, (0, 0, 1), layers=2, vacuum=10.0)
    shifted = sl.build_slab(cell, (0, 0, 1), layers=2, vacuum=10.0, shift=0.25)
...
def test_slab_shift_keeps_the_atom_count():
    ...
    for shift in (0.0, 0.1, 0.25, 0.5, 0.75, 0.9):
        slab = sl.build_slab(cell, (0, 0, 1), layers=3, vacuum=10.0, shift=shift)
...
def test_slab_shift_is_a_no_op_for_a_single_plane_basis(simple_cubic):
    ...
    plain = sl.build_slab(simple_cubic, (0, 0, 1), layers=2, vacuum=10.0)
    shifted = sl.build_slab(simple_cubic, (0, 0, 1), layers=2, vacuum=10.0, shift=0.5)
    assert _stacking_pattern(plain) == _stacking_pattern(shifted)


def test_slab_shift_wraps_past_one(simple_cubic):
    a = sl.build_slab(simple_cubic, (0, 0, 1), layers=2, vacuum=10.0, shift=0.25)
    b = sl.build_slab(simple_cubic, (0, 0, 1), layers=2, vacuum=10.0, shift=1.25)
...
def test_write_cif_roundtrips_through_the_parser(simple_cubic):
    slab = sl.build_slab(simple_cubic, (1, 1, 0), layers=2, vacuum=12.0)
